...
control_panel = [
    tile("hq_hc", "Численность и рост", "Численность HQ (управленческая)", "чел.", "int",
         ...
         status_text="Прогноз по воронке найма — 23 620 к декабрю: +120 над лимитом",
         owner="Директор по персоналу HQ", page="headcount",
         question="Какие ~300 новых вакансий HQ сдвигаем на 2027, чтобы уложиться в лимит?",
...
headcount = {
    ...
    "forecast": [
        ...
        {"id": "cap", "label": "Чтобы уложиться в лимит: не более ~1 070 внешних наймов за сен–дек",
    ...
    "questions": [
        {"q": "Лимит 23 500 достижим только если в IV квартале нанимать на треть меньше, чем сейчас. Кто принимает решение о приоритизации 1 850 вакансий?", "to": "HR HQ, финансы"},
    ...
    "comment": ("HQ выросла на 790 человек с начала года и идёт чуть выше плановой траектории. "
                ...
                "сделать не более ~1 070 внешних наймов против ~1 230 при текущем темпе."),
...
summary = {
    ...
    "paragraphs": [
        ...
        "если в IV квартале нанимать примерно на треть меньше, чем сейчас. Решение о приоритизации вакансий нужно в сентябре.",
    ...
    "signals": [
        {"level": "red", "title": "Лимит численности HQ", "text": "Прогноз по воронке 23 620 к декабрю (+120). Нужно решение по ~300 вакансиям.", "page": "headcount", "owner": "HR HQ, финансы"},
    ...
    "questions": [
        {"q": "Какие ~300 вакансий HQ переносим на 2027 и кто владелец этого решения?", "to": "HR HQ, финансы, руководители линий", "page": "headcount"},
...
findings = [
    {
        "id": "hidden_growth",
        "kicker": "Скрытый рост",
        "claim": "Восемьдесят человек, которых нет ни в одном плане",
        "value": 79, "unit": "замен", "value_note": "закрыты при работающем «заменяемом»",
        "text": ("В 720 вакансиях на замену мы сверили статус того, кого заменяют. У 79 замена уже вышла на работу, "
                 ...
                 "Фактически это плюс восемьдесят человек к численности HQ."),
    ...
    },
    {
        ...
        "question": "Это реальная передача функций или способ уместиться в лимит?",
...
stakes = {
    ...
    "items": [
        {"label": "Превышение лимита", "value": "120 человек", "money": "≈ 0,5 млрд ₽ в год",
         "assumption": "оценка: средние затраты на сотрудника HQ 4,2 млн ₽ в год"},
        {"label": "Скрытый рост через замены", "value": "80 человек", "money": "≈ 0,3 млрд ₽ в год",
         "assumption": "то же допущение"},
